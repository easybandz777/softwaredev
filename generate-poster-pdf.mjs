...
const OUTPUT_PATH = path.join(__dirname, 'QuantLab_Poster_48x30.pdf');
...
async function generatePDF() {
    ...
    // Viewport at native poster size (48in x 30in @ 96dpi) with 2x for high-res
    await page.setViewport({ width: 4608, height: 2880, deviceScaleFactor: 2 });

    console.log('Loading clean print page...');
    await page.goto('http://localhost:3000/print/poster-print', {
        waitUntil: 'networkidle0',
        timeout: 60000,
    });

    // Force-hide root layout elements (Navbar, FuturisticBackground, PageTracker)
    await page.addStyleTag({
        content: `
            /* Hide ALL elements before the poster */
            body > *:not(#poster):not(:has(#poster)) { display: none !important; height: 0 !important; overflow: hidden !important; }
            /* Make sure poster's parent chain is visible */
    ...
    // Screenshot the #poster element directly (no wrapper to worry about)
    ...
    const screenshotPath = path.join(__dirname, '_tmp_poster.png');
    console.log('Capturing high-res screenshot of poster...');
    ...
    // Build a clean PDF with the screenshot embedded at correct physical size
